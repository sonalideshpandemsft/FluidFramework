	...
	private async load(
		...
		switch (opsBeforeReturn) {
			...
			case "sequenceNumber": {
				...
				// TODO: Historical op replay happens through DeltaManager.attachOpHandler("sequenceNumber")
				// and processRemoteMessage; if we need explicit batchId-target replay semantics, add it in
				// that pipeline and keep this gate as the pause-at-target coordination point.
				const targetReachedP = new Promise<void>((resolve, reject) => {
					...
					const targetReachedCloseHandler = (error?: ICriticalContainerError): void => {
						this.off("op", targetReachedHandler);
						this.off("closed", targetReachedCloseHandler);
						reject(
							normalizeError(
