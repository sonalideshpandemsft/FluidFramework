...
describe("OdspFileVersionFetcher (integration, stubbed fetch)", () => {
	...
	it("resolveLiveLatestSequenceNumber reads a fresh unversioned snapshot head", async () => {
		const { result, urls } = await withFetch(
			[await createResponse(jsonHeaders, snapshotWithSeq(448, 455), 200)],
			async () => fetcher.resolveLiveLatestSequenceNumber(),
		);

		assert.equal(result, 455);
		assert.ok(
