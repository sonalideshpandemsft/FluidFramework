...
import { createPointInTimeAvailabilityProviderCore } from "../pointInTimeDriver/createPointInTimeAvailabilityProvider.js";
import type {
...
describe("createPointInTimeAvailabilityProvider", () => {
	...
	it("returns conclusive base results without connecting delta storage", async () => {
		...
		assert.deepEqual(result, [
			...
			{ sequenceNumber: 20, status: "unavailable", reason: "lineageMismatch" },
	...
	it("requires all interleaved chunk streams to complete before reporting a boundary", async () => {
		const chunk = (
			sequenceNumber: number,
			clientId: string,
			chunkId: number,
