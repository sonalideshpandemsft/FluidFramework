...
of a chunked operation are rejected. A bounded stream that reaches `done` has therefore
...
caught as reads happen and the load fails loudly instead of returning a wrong document. Availability
uses the full manager's explicit base/live epoch comparison because it must classify lineage mismatch
without instantiating document services. A fresh
...
inversions cannot hide a closer base, and lineage-checks the selected retained bases,
reads an unversioned live snapshot directly to establish a fresh storage-head observation, and
...
`unavailable`. The delta-validation service is never connected to storage, so it cannot use bundled
live-snapshot ops that the actual point-in-time load discards. Targets inside a runtime batch or an
incomplete chunk stream are authoritatively `unavailable` with `notMaterializationBoundary`.
Discovery failures (including restore races) and the absence of any resolved sealed version are
`unknown`; only a target older than a known oldest sealed base is `noRetainedBase`. Version sequence
