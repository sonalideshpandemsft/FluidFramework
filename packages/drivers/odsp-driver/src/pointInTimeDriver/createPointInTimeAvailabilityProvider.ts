...
/* eslint-disable import-x/no-internal-modules -- The provider consumes legacy public types and internal version management. */
import type {
...
function isMissingOpsError(error: unknown): boolean {
	return (
		(error as { errorType?: unknown } | undefined)?.errorType === OdspErrorTypes.cannotCatchUp
	);
}
...
function classifyFailure(
	...
	if (errorType === OdspErrorTypes.fileOverwrittenInStorage) {
		return unavailable(sequenceNumber, "lineageMismatch");
	}
		...
		while (true) {
			...
			for (const message of result.value) {
				if (message.sequenceNumber !== nextExpected) {
					return { nextExpected, invalidBoundaries };
				}
...
/**
 * Creates ODSP's document-bound point-in-time availability provider.
 *
 * @param props - The resolved document, storage token fetcher, cache, and live service factory.
 * @returns A provider bound to the resolved ODSP document.
 *
 * @legacy @beta
 */
export async function createPointInTimeAvailabilityProvider(
	props: OdspPointInTimeAvailabilityImplementationProps,
): Promise<PointInTimeAvailabilityProvider> {
	return createPointInTimeAvailabilityProviderCore(props);
}

/**
 * Creates the provider with replaceable dependencies for testing.
 *
 * @internal
 */
export async function createPointInTimeAvailabilityProviderCore(
	props: OdspPointInTimeAvailabilityImplementationProps,
	...
	return {
		checkSequenceNumberAvailability: async (sequenceNumbers, options) => {
			...
			try {
				availabilityBases = await versionManager.findBasesForSeqs(
					sequenceNumbers,
					options?.signal,
				);
			} catch {
			...
			for (let index = 0; index < sequenceNumbers.length; index++) {
				...
				} else if (base.kind === "lineageMismatch") {
					results[index] = unavailable(target, "lineageMismatch");
			...
			try {
				...
				for (const { from, indexes } of groups.values()) {
					...
					const verification = await verifyRange(deltaStorage, from, to, options?.signal);
					for (const index of indexes) {
						const target = sequenceNumbers[index];
						if (target < verification.nextExpected) {
							...
							results[index] = classifyFailure(target, verification.error, options?.signal);
						}
					}
				}
			} catch (error) {
				for (const { indexes } of groups.values()) {
					for (const index of indexes) {
						results[index] ??= classifyFailure(sequenceNumbers[index], error, options?.signal);
					}
				}
			} finally {
				liveService?.dispose();
